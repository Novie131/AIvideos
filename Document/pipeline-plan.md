...
1. **可展示的技術成果** —— 讀者是工程師與技術主管
2. **產出的影片發到 YouTube Shorts**，驗證內容與變現可能
3. 驗證這個題材到底有沒有觀眾

**釐清過程中確認的一件事**：這個題材**沒有可參照的同類頻道數據**，整份計畫建立在推測上。
這決定了第一支片的角色 —— **它是探針，不是作品**。
...
1. 沒有同類頻道數據 → 需要真實數據 → 得先發一支
2. 可展示的成果需要看得見的東西 → 得先有一支
3. 一條沒跑通的產線，規劃再完整也無法驗證 → 得先有一支

前提不同，結論相同。

### ADR-011 — 內容分兩軌：故事劇情 / 才藝展示 ⬆️ 新增（2026-09-24）

**背景**：第一支片（ep001）端到端完成後，使用者看片的回饋是「沒有流暢感，像放簡報」。
同時提出未來內容分兩軌：**故事劇情**（連貫短劇，每集約 30 秒）與**才藝展示**（跳舞、唱歌）。
並且明確表示**時程不急**，ADR-010 的 2026-10-06 期限因此解除。

**決策**：採用兩軌，但**技術難度不對等，實作順序不能照直覺走**。

| | 故事劇情 | 才藝展示 |
|---|---|---|
| 腳本結構 | 鏡頭序列（跟劇情走） | **節拍序列**（跟音樂走） |
| 動作需求 | 低 —— 眨眼、尾巴、光影、風 | **高 —— 指定的舞步** |
| I2V 可行性 | 文字驅動 I2V 正好擅長 | 文字驅動**做不到指定動作** |
| 現況 | ep001 已完成、ep002 腳本現成 | 三個未解的硬問題（見下） |

**才藝軌的三個硬問題**（都不是「多花時間就能解」）：

1. **四足貓做不了人類舞步。** 韓團舞、街舞都是雙足人類動作，貓的骨架做不到。
   業界能指定舞步的技術（AnimateAnyone / MimicMotion / Champ）原理是從參考影片抽**人體骨架**再驅動角色，
   貓身上套不上。➡️ **要跳舞，小橘就得有一個擬人雙足版本**，這與 `character.json` 現行設定衝突。
2. **音樂版權直接殺掉分潤。** 用真實 K-pop 歌曲 → Content ID 認領 → 該片不能營利，與目的 #2 抵觸。
   ➡️ 才藝軌只能用 YouTube 音訊庫 / 免版權 / 自生音樂，所以是「跳一段舞」不是「跳某團的舞」。
3. **文字驅動 I2V 給不了指定動作。** 只能下 "a cat dancing"，出來什麼由模型決定。

**實作順序**：

1. **先用故事軌驗證 I2V** —— 用 ep001 的現成圖測，不需要新素材。
   故事片需要的正是泛泛的自然動作，是文字驅動 I2V 的強項。
   **I2V 在這台機器上跑不動的話，才藝軌連討論的必要都沒有。**
2. 才藝軌開工前要先決定兩件事（設定問題，不是技術問題）：
   - 開一個擬人雙足的小橘（`characters/orange-cat-biped/`），或改成貓做得到的才藝（彈琴、打鼓、疊杯）
   - 音樂來源

**現在就要做的架構改動**（零成本，事後補要回頭改所有腳本）：
`script.json` 增加 `type` 欄位（`"story"` / `"performance"`），兩者結構不同 ——
故事是鏡頭序列，才藝是節拍序列。`scriptgen.py` 的編劇提示詞要分兩套。
`projects/ep002-egypt-30s/`（27 秒 9 鏡頭）可直接當故事軌的第一個樣本。

### ADR-012 — 動作感靠「切」與「出圖姿勢」，不靠 I2V 參數 ⬆️ 新增（2026-09-25）

**背景**：ep001 完成後使用者回饋「像放簡報」。先調了 render.py 的運鏡（幅度 1.15→1.28、
加 smoothstep 緩動、加 0.3s 方向性轉場），使用者再看仍說「完全沒有在動」。
於是實測 I2V（Wan 2.2 TI2V 5B，Q5 因此解決 —— 可行，384x640/25 影格/25 步約 128 秒）。

**關鍵實測：I2V 的動作幅度有硬上限，而且代價曲線很差** `[已驗證]`

| strength | 首格偏離原圖 | 逐格動作量 | |
|---|---|---|---|
| 0.60 | 21.4 | 4.19 | 基準 |
| 0.85 | 26.6 | 5.55 | 失真 +25%，動作 +32% |
| 0.95 | **41.0** | 6.44 | 失真 **+92%**，動作僅 +54% |

即使把提示詞換成純動作動詞、`motion_scale` 拉到 255、影格加到 49，
動作量也只從 4.19 爬到 6.44。而 strength 0.95 的畫面裡
**項圈與黃銅鈴鐺整個消失** —— 那是 §3 指定的兩個驗收點之一。

**根本原因**：Wan 2.2 TI2V 5B 做的是「既有姿勢的自然延續」。
它讓一隻站著的貓「站著但有呼吸」，**它不會讓一隻站著的貓開始跑**。

**決策**：動作感的來源重新排序，參數調校降到最後一位。

1. **出圖階段就把動作凍結進畫面**（免費，影響最大）
   `image_prompt` 必須是「動作進行到一半」的瞬間，不是擺拍。
   `scriptgen.py` 新增 `MOTION_CUES` 清單與對應硬檢查：
   image_prompt 不含任何動態線索就判定不通過。
2. **鏡頭砍短，用「切」給節奏**（免費）
   `SEC_PER_SHOT` 3.2 → **1.8**，`check()` 的單鏡頭上限 4 秒 → **2.5 秒**，
   `suggest_shots` 上限 12 → 20。15 秒從 5 格變 8 格。
3. **相鄰鏡頭做動作連續性**（免費）
   一個大動作拆成連續 2~3 格（蓄力 → 騰空 → 落地），靠觀眾腦補連成流暢動作。
4. **不是每格都要旁白**（順帶解決 ep001 旁白只有 5.75 秒 / 影片 15 秒的問題）
   鏡頭變多後句句都講會變連珠砲，留白是節奏的一部分。
5. **I2V 定位改變**：不是「讓靜圖動起來」的魔法，是「幫已經有動態的畫面加呼吸感」的加分項。
   用 **strength 0.85**（0.95 會毀掉角色）。

**對才藝軌的影響**：這次實測證實了 ADR-011 的判斷 ——
這個動作幅度做得了故事軌（眨眼、尾巴、塵埃、光影），**絕對做不了舞蹈**。
才藝軌不是參數問題，需要不同的技術路線。

**I2V 的三個 API 坑**（實測踩到，寫在這裡免得重踩）：

1. `init_images` 的尺寸必須與 `width`/`height` **完全相同**，Draw Things 不會自己縮放，不符回 422。
2. **Wan 要求長寬可被 32 整除。** 送 672 進去**不報錯**，Draw Things 悄悄改成 640，
...
| **M6** | 案例研究頁 | 一頁案例研究：問題、硬約束、架構決策、成品嵌入 | 讀者是工程師與技術主管。主角是**判斷力與收尾能力**，不是操作截圖 —— 截圖滿網路都是，那份 ADR 文件才是稀有的 |
